<x-app-layout>
                        ...
                        <td class="px-5 py-3">
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-400">{{ $entry->rentShop->rentMarket->name ?? '—' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-5 py-3">
    ...
    @can('manage rent')
    <div id="modal-add-rent" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between p-5 border-b border-slate-100 sticky top-0 bg-white rounded-t-2xl">
                ...
                <button onclick="document.getElementById('modal-add-rent').classList.add('hidden')" class="text-slate-400 hover:text-slate-600 w-8 h-8 rounded-lg hover:bg-slate-100 flex items-center justify-center"><i class="fas fa-times"></i></button>
            </div>
            <form method="POST" action="{{ route('rent.store') }}" class="p-5 space-y-4">
                ...
                <div>
                    ...
                    <select id="rent-market-filter" onchange="filterRentShops(this)"
                ...
                <div>
                    ...
                    <select name="rent_shop_id" id="rent-shop-select" required onchange="fillShopNumber(this)"
                            class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
                        <option value="">— Select Shop —</option>
                        @foreach($shops as $shop)
                        <option value="{{ $shop->id }}" data-number="{{ $shop->shop_number }}" data-market="{{ $shop->rent_market_id }}">
                            {{ $shop->rentMarket->name }} – Shop #{{ $shop->shop_number }}
                        </option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <input type="text" name="shop_number" id="rent-shop-number" required
                           class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" placeholder="Auto-filled">
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" id="rent-customer-search" autocomplete="off" placeholder="Search customer..."
                               class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
                               oninput="rentSearch(this)" onfocus="rentSearch(this)">
                        <input type="hidden" name="customer_id" id="rent-customer-id">
                        <div id="rent-customer-dd" class="hidden absolute z-50 w-full bg-white border border-slate-200 rounded-xl shadow-xl mt-1 max-h-52 overflow-y-auto"></div>
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Received By</label>
                    <input type="text" name="received_by" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" placeholder="Name of collector">
                </div>
                <div>
    ...
    <script>
    const rentCustomers = <?php echo json_encode($customers->map(fn($c)=>['id'=>$c->id,'name'=>$c->name,'phone'=>$c->phone??'','cnic'=>$c->cnic??''])->values()); ?>;

    function rentEsc(s){ return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/'/g,"\\'"); }

    function rentSearch(input){
        const q = input.value.trim().toLowerCase();
        const dd = document.getElementById('rent-customer-dd');
        const list = q ? rentCustomers.filter(r=>r.name.toLowerCase().includes(q)||r.phone.includes(q)) : rentCustomers.slice(0,25);
        dd.innerHTML = list.length
            ? list.map(r=>`<div class="flex items-center justify-between px-4 py-2.5 hover:bg-indigo-50 cursor-pointer border-b border-slate-50 last:border-0"
                  onmousedown="rentPick(${r.id},'${rentEsc(r.name)}','${rentEsc(r.phone)}')">
                  <div><p class="font-medium text-slate-800 text-sm">${r.name}</p>${r.cnic?`<p class="text-xs text-slate-400">${r.cnic}</p>`:''}</div>
                  ${r.phone?`<span class="text-xs text-indigo-600">${r.phone}</span>`:''}
              </div>`).join('')
            : '<div class="px-4 py-3 text-sm text-slate-400">No results</div>';
        dd.classList.remove('hidden');
    }

    function rentPick(id, name, phone){
        document.getElementById('rent-customer-id').value = id;
        document.getElementById('rent-customer-search').value = name + (phone?' — '+phone:'');
        document.getElementById('rent-customer-dd').classList.add('hidden');
    }

    function fillShopNumber(select){
        document.getElementById('rent-shop-number').value = select.options[select.selectedIndex].dataset.number || '';
    }

    function filterRentShops(marketSelect){
        const mid = marketSelect.value;
        Array.from(document.getElementById('rent-shop-select').options).forEach(opt=>{
            if(!opt.value) return;
            opt.style.display = (!mid || opt.dataset.market == mid) ? '' : 'none';
        });
        document.getElementById('rent-shop-select').value = '';
        document.getElementById('rent-shop-number').value = '';
    }

    document.addEventListener('click', e=>{
        const dd=document.getElementById('rent-customer-dd'), inp=document.getElementById('rent-customer-search');
        if(dd&&inp&&!inp.contains(e.target)&&!dd.contains(e.target)) dd.classList.add('hidden');
    });
    </script>
</x-app-layout>
